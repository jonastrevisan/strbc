 ...
 *  - Any of the normal configs will be passed through to either component. - There can be a docked config. - The timepicker can be on the right or left (datepicker, too, of course). - Choose which
 * component will initiate the filtering, i.e., the event can be configured to be bound to either the datepicker or the timepicker, or if there is a docked config it be automatically have the handler
 * bound to it.
 * 
 * Although not shown here, this class accepts all configuration options for {@link Ext.picker.Date} and {@link Ext.picker.Time}.
 * 
 * In the case that a custom dockedItems config is passed in, the class will handle binding the default listener to it so the developer need not worry about having to do it.
 * 
 * The default dockedItems position and the toolbar's button text can be passed a config for convenience, i.e.,:
 * 
 ...
 * Example Usage:
 * 
 * var filters = Ext.create('Ext.ux.grid.GridFilters', { ... filters: [{ // required configs type: 'datetime', dataIndex: 'date',
 *  // optional configs positionDatepickerFirst: false, //selectDateToFilter: false, // this is overridden b/c of the presence of the dock cfg object
 * 
 * date: { format: 'm/d/Y', },
 * 
 ...
 * 
 * In the above example, note that the filter is being passed a {@link #date} config object, a {@link #time} config object and a {@link #dock} config. These are all optional.
 * 
 * As for positioning, the datepicker will be on the right, the timepicker on the left and the docked items will be docked on the left. In addition, since there's a {@link #dock} config, clicking the
 * button in the dock will trigger the filtering.
 */
Ext.define('Ext.ux.grid.filter.DateTimeFilter', {
    ...
    /**
	 * @cfg {Object} date A {@link Ext.picker.Date} can be configured here. Uses {@link #dateDefaults} by default.
	 */

    /**
	 * @cfg {Object} time A {@link Ext.picker.Time} can be configured here. Uses {@link #timeDefaults} by default.
	 */

    /**
	 * @cfg {Boolean/Object} dock A {@link Ext.panel.AbstractPanel#cfg-dockedItems} can be configured here. A `true` value will use the {@link #dockDefaults} default configuration. If present, the
	 *      button in the docked items will initiate the filtering.
	 */

    /**
	 * @cfg {Boolean} [selectDateToFilter=true] By default, the datepicker has the default event listener bound to it. Setting to `false` will bind it to the timepicker.
	 * 
    ...
    /**
	 * @cfg {Boolean} [positionDatepickerFirst=true] Positions the datepicker within its container. A `true` value will place it on the left in the container. Set to `false` if the timepicker should
	 *      be placed on the left. Defaults to `true`.
	 */
    ...
    addTimeSelection : function(date, timepicker) {
	    var me = this, selection = timepicker.getSelectionModel().getSelection(), time, len, fn, val, i = 0, arr = [], timeFns = [ 'setHours', 'setMinutes', 'setSeconds', 'setMilliseconds' ];
    ...
    init : function(config) {
	    var me = this, dateCfg = Ext.applyIf(me.date || {}, me.dateDefaults), timeCfg = Ext.applyIf(me.time || {}, me.timeDefaults), dockCfg = me.dock, // should not default to empty object
	    defaultListeners = {
	        click : {
	            scope : me,
	    ...
	    if (!dockCfg) {
		    ...
		    if (dockCfg.dockedItems) {
			    ...
			    //  allow config that will tell which item to bind the listener to
    ...
    onCheckChange : function(item, checked) {
	    var me = this, menu = item.menu, timepicker = menu.down('timepicker'), datepicker = menu.down('datepicker'), itemId = datepicker.itemId, values = me.values;
    ...
    onMenuSelect : function(picker, date) {
	    ...
	    var me = this, menu = me.menu, checkItemId = menu.getFocusEl().itemId.replace(me.reItemId, '$1'), fields = me.fields, field;
    ...
    getSerialArgs : function() {
	    var me = this, key = 0, fields = me.fields, args = [];
	 ...
	 * @param {Boolean}
	 *            preserve true to preserve the checked status of the other fields. Defaults to false, unchecking the other fields
	 */
    setValue : function(value, preserve) {
	    var me = this, fields = me.fields, key = 0, val, datepicker;
    ...
    validateRecord : function(record) {
	    ...
	    var me = this, key = 0, pickerValue, val = record.get(me.dataIndex);
