...
if (empty($value) || $value == '') {
    ...
    $queryString = "SELECT * FROM socio where ativo = 'S' ORDER BY matricula ASC LIMIT $start,  $limit";
    ...
    $queryTotal = $conecta->query("SELECT count(matricula) as num FROM socio where ativo = 'S'") or die(mysql_error());
    ...
    $queryTotal = $conecta->query($queryNumber) or die(mysql_error()); // and $field = $value
    $row = $queryTotal->fetch_assoc();
    $total = $row['num'];
